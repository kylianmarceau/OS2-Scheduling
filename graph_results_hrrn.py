#!/usr/bin/env python3
"""
Generate the bonus HRRN comparison graphs.

This script is separate from graph_results.py on purpose:
- it reads the original scheduler CSVs from results/
- it reads the bonus HRRN CSV from hrrn_results/
- it writes only to hrrn_graphs/

Your existing CSVs and original graph PNGs are not modified.
"""
...
BASE_SCHEDULERS = ["FCFS", "SJF", "PRIORITY", "MLFQ"]
SCHEDULERS = ["FCFS", "SJF", "PRIORITY", "MLFQ", "HRRN"]
COLORS = {
    "FCFS": "#2563eb",
    "SJF": "#059669",
    "PRIORITY": "#f97316",
...
def read_scheduler_csv(csv_file, expected_scheduler):
    if not csv_file.exists():
        raise FileNotFoundError(
            f"Missing {expected_scheduler} results file: {csv_file}\n"
            f"Run ./run_hrrn_experiments.sh first if HRRN.csv is missing."
        )
    ...
    schedulers_found = set(frame["scheduler"].dropna().unique())
    if schedulers_found != {expected_scheduler}:
        raise ValueError(
            f"{csv_file} should contain only scheduler {expected_scheduler}, "
            f"but found {sorted(schedulers_found)}"
        )

    frame["sourceFile"] = csv_file.name
    frame["csvLineNumber"] = frame.index + 2
    return frame


def load_results(results_dir, hrrn_results_dir):
    frames = []
    for scheduler in BASE_SCHEDULERS:
        frames.append(read_scheduler_csv(results_dir / f"{scheduler}.csv", scheduler))
    frames.append(read_scheduler_csv(hrrn_results_dir / "HRRN.csv", "HRRN"))

    data = pd.concat(frames, ignore_index=True)
    for column in NUMERIC_COLUMNS:
        data[column] = pd.to_numeric(data[column], errors="raise")

    data["scheduler"] = pd.Categorical(data["scheduler"], SCHEDULERS, ordered=True)
    validate_results(data)
    validate_matching_experiments(data)
    return data
...
def validate_results(data):
    ...
    if invalid_wait.any():
        raise ValueError(format_invalid_rows(
            data,
            invalid_wait,
            "At least one row has a waitingTime inconsistent with timestamps.",
        ))
    if invalid_response.any():
        raise ValueError(format_invalid_rows(
            data,
            invalid_response,
            "At least one row has a responseTime inconsistent with timestamps.",
        ))
    if invalid_turnaround.any():
        raise ValueError(format_invalid_rows(
            data,
            invalid_turnaround,
            "At least one row has a turnaroundTime inconsistent with timestamps.",
        ))
    if invalid_order.any():
        print(format_invalid_rows(
            data,
            invalid_order,
            "Warning: at least one row has completionTime earlier than serviceStartTime.",
            suggest_regeneration=False,
        ))


def format_invalid_rows(data, mask, message, suggest_regeneration=True):
    columns = [
        "sourceFile",
        "csvLineNumber",
        "runId",
    ...
    bad_rows = data.loc[mask, columns].head(8).to_string(index=False)
    output = (
        f"{message}\n\n"
        f"Bad row sample:\n{bad_rows}"
    )
    if suggest_regeneration:
        output += (
            "\n\nThis means the HRRN CSV needs to be regenerated before graphing. "
            "Run ./run_hrrn_experiments.sh, then run python3 graph_results_hrrn.py again."
        )
    else:
        output += (
            "\n\nContinuing because waitingTime, responseTime, and turnaroundTime "
            "are still validated separately and are the values used in the graphs."
        )
    return output


def validate_matching_experiments(data):
    run_counts = (
        data.groupby("scheduler", observed=True)["runId"]
        .nunique()
        .reindex(SCHEDULERS)
    )
    if run_counts.isna().any() or (run_counts == 0).any():
        missing = run_counts[run_counts.isna() | (run_counts == 0)].index.tolist()
        raise ValueError(f"Missing run data for schedulers: {missing}")

    parameter_sets = {
        scheduler: set(
            data.loc[data["scheduler"] == scheduler, ["noPatrons", "switchTime", "seed"]]
            .drop_duplicates()
            .itertuples(index=False, name=None)
        )
        for scheduler in SCHEDULERS
    }
    expected = parameter_sets[SCHEDULERS[0]]
    mismatched = [scheduler for scheduler, values in parameter_sets.items() if values != expected]
    if mismatched:
        raise ValueError(
            "The HRRN experiment set does not match the original scheduler experiment set. "
            f"Mismatched schedulers: {mismatched}"
        )
...
def build_run_summary(data):
    ...
    for run_key, run_data in data.groupby(group_columns, observed=True, sort=False):
    ...
    run_summary["scheduler"] = pd.Categorical(run_summary["scheduler"], SCHEDULERS, ordered=True)
    patron_summary["scheduler"] = pd.Categorical(patron_summary["scheduler"], SCHEDULERS, ordered=True)
...
def build_load_summary(run_summary):
    ...
    summary = (
        run_summary
        .groupby(["scheduler", "noPatrons"], observed=True, as_index=False)[value_columns]
        .mean()
    )
...
def plot_core_metric_group(load_summary, output_dir, metric_type):
    if metric_type == "mean":
        ...
        filename = "01_core_metrics_mean_with_hrrn.png"
        ...
        filename = "02_core_metrics_median_with_hrrn.png"
    else:
...
def plot_distributions(data, patron_summary, output_dir):
    fig, axes = plt.subplots(2, 2, figsize=(12.8, 7.8))
    ...
    save_figure(fig, output_dir / "03_core_metric_distributions_with_hrrn.png")
...
def plot_throughput(run_summary, load_summary, output_dir):
    ...
    fig.legend(handles=scheduler_handles(), loc="lower center", ncol=len(SCHEDULERS), bbox_to_anchor=(0.5, -0.02))
    fig.tight_layout(rect=(0, 0.08, 1, 1), w_pad=2.0)
    save_figure(fig, output_dir / "04_throughput_with_hrrn.png")
...
def plot_quality_metrics(load_summary, output_dir):
    ...
    fig.legend(handles=scheduler_handles(), loc="lower center", ncol=len(SCHEDULERS), bbox_to_anchor=(0.5, -0.01))
    fig.tight_layout(rect=(0, 0.06, 1, 1), h_pad=2.0, w_pad=2.0)
    save_figure(fig, output_dir / "05_predictability_fairness_starvation_with_hrrn.png")
...
def plot_metric_summary_table(run_summary, output_dir):
    metrics = [
        ...
        ("Order-level timing", "waitingMean", "Mean waiting time per order (ms)", "mean"),
        ("Order-level timing", "waitingMedian", "Median waiting time per order (ms)", "mean"),
        ("Order-level timing", "responseMean", "Mean response time per order (ms)", "mean"),
        ("Order-level timing", "responseMedian", "Median response time per order (ms)", "mean"),
        ("Order-level timing", "turnaroundMean", "Mean turnaround time per order (ms)", "mean"),
        ("Order-level timing", "turnaroundMedian", "Median turnaround time per order (ms)", "mean"),
        ("Predictability", "turnaroundStd", "Per-order turnaround std. dev. (ms)", "mean"),
        ...
        ("Patron-level fairness", "patronWaitingStd", "Total patron waiting std. dev. (ms)", "mean"),
    ...
    for group, column, label, _ in metrics:
        if group != previous_group:
            ...
            cell_text.append([group] + [""] * len(SCHEDULERS))
...
def print_required_summary(data, run_summary, output_dir):
    print(f"Read {len(data)} completed drink orders across FCFS, SJF, PRIORITY, MLFQ, and HRRN.")
    print(f"Summarised {len(run_summary)} runs.")
    print("Wrote HRRN comparison graph/table set:")
    print(f"  {output_dir / '01_core_metrics_mean_with_hrrn.png'}")
    print(f"  {output_dir / '02_core_metrics_median_with_hrrn.png'}")
    print(f"  {output_dir / '03_core_metric_distributions_with_hrrn.png'}")
    print(f"  {output_dir / '04_throughput_with_hrrn.png'}")
    print(f"  {output_dir / '05_predictability_fairness_starvation_with_hrrn.png'}")
    print(f"  {output_dir / '06_metric_summary_table_with_hrrn.png'}")


def parse_args():
    parser = argparse.ArgumentParser(description="Generate HRRN bonus comparison graphs.")
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("results"),
        help="Directory containing FCFS/SJF/PRIORITY/MLFQ CSV files. Default: results",
    )
    parser.add_argument(
        "--hrrn-results-dir",
        type=Path,
        default=Path("hrrn_results"),
        help="Directory containing HRRN.csv. Default: hrrn_results",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("hrrn_graphs"),
        help="Directory where HRRN comparison PNGs are written. Default: hrrn_graphs",
    )
...
def main():
    ...
    data = load_results(args.results_dir, args.hrrn_results_dir)
    ...
    print_required_summary(data, run_summary, args.output_dir)
